...
export class ImageProcessor {
  static async createThumbnail(imageBuffer: ArrayBuffer, maxWidth: number = 400): Promise<{
    ...
    const thumbnailWidth = Math.min(maxWidth, imageBitmap.width);
    const thumbnailHeight = Math.round(thumbnailWidth / aspectRatio);
    
    // Resize canvas and draw thumbnail
    canvas.width = thumbnailWidth;
    canvas.height = thumbnailHeight;
    ctx.drawImage(imageBitmap, 0, 0, thumbnailWidth, thumbnailHeight);
    
    // Get image data for thumbhash
    const imageData = ctx.getImageData(0, 0, thumbnailWidth, thumbnailHeight);
    const rgba = new Uint8Array(imageData.data);
    
    // Generate thumbhash
    const thumbhash = rgbaToThumbHash(thumbnailWidth, thumbnailHeight, rgba);
    const thumbhashString = Array.from(thumbhash, (byte: any) => byte.toString(16).padStart(2, '0')).join('');
    ...
    return {
      ...
      metadata: { width: thumbnailWidth, height: thumbnailHeight }
    };
  }

  static thumbhashToDataURL(thumbhash: string): string {
    const bytes = new Uint8Array(thumbhash.match(/.{2}/g)!.map(byte => parseInt(byte, 16)));
...
export class AuthManager {
  ...
  async initializeAdmin(email: string, password: string): Promise<void> {
    ...
    await this.env.KV.put(KV_KEYS.ADMIN_CREDENTIALS, JSON.stringify(credentials));
  }
  ...
  async authenticateAdmin(email: string, password: string): Promise<boolean> {
    ...
    return credentials.email === email && await this.verifyPassword(password, credentials.password);
  }
  ...
  async createSession(email: string): Promise<string> {
    ...
    await this.env.KV.put(`${KV_KEYS.SESSION_PREFIX}${sessionId}`, JSON.stringify(session));
    return sessionId;
  }

  async validateSession(sessionId: string): Promise<AuthSession | null> {
    const sessionJson = await this.env.KV.get(`${KV_KEYS.SESSION_PREFIX}${sessionId}`);
...
export class PhotoManager {
  ...
  async uploadPhoto(
    ...
    const { thumbnailBuffer, thumbhash, metadata } = await ImageProcessor.createThumbnail(file);
    ...
    await this.env.KV.put(`${KV_KEYS.PHOTO_PREFIX}${photoId}`, JSON.stringify(photo));
  ...
  private async addToPhotoList(photoId: string, timestamp: string): Promise<void> {
  ...
  async getPhotos(cursor?: string, limit: number = 20): Promise<{
    photos: Photo[];
    nextCursor?: string;
    hasMore: boolean;
  }> {
    ...
    for (const photoId of photoIds) {
      const photoJson = await this.env.KV.get(`${KV_KEYS.PHOTO_PREFIX}${photoId}`);
      if (photoJson) {
  ...
  async searchPhotos(query: string, tags?: string[], limit: number = 20): Promise<Photo[]> {
    const listJson = await this.env.KV.get(KV_KEYS.PHOTO_LIST);
    if (!listJson) return [];
